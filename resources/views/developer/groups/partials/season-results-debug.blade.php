...
<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-6" data-season-results-container data-mode="{{ $resultsMode }}" data-endpoint="{{ route('developer.groups.season-results', ['group' => $group->ulid]) }}" data-highlight-player-ids='@json($highlightPlayerIds)'>
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h3 class="text-lg font-semibold text-gray-900">
                {{ $resultsMode === 'leaderboard' ? 'Leaderboard results' : 'Raw prediction results' }}
            </h3>
            <p class="text-sm text-gray-600">Season-scoped scoring output for this group.</p>
        </div>
    ...
    @else
        ...
        <div data-results-meta class="mb-4 hidden rounded-md border border-gray-200 bg-gray-50 p-4 text-xs text-gray-700"></div>
        ...
        <script>
            (() => {
                ...
                const metaState = container.querySelector('[data-results-meta]');
                ...
                const renderMeta = (data) => {
                    ...
                    const excludedCount = Array.isArray(meta.excluded_games) ? meta.excluded_games.length : 0;

                    metaState.innerHTML = `
                        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                            <div><span class="font-semibold">Policy:</span> ${escapeHtml(data.points_policy ?? '-')}</div>
                            <div><span class="font-semibold">Generated:</span> ${escapeHtml(data.generated_at ?? '-')}</div>
                            <div><span class="font-semibold">Season games loaded:</span> ${escapeHtml(meta.total_season_games_loaded ?? 0)}</div>
                            <div><span class="font-semibold">Prediction rows loaded:</span> ${escapeHtml(meta.total_predictions_loaded ?? 0)}</div>
                            <div><span class="font-semibold">Excluded games:</span> ${escapeHtml(excludedCount)}</div>
                            <div class="sm:col-span-2 lg:col-span-3"><span class="font-semibold">Notes:</span> ${escapeHtml(notes || '-')}</div>
                        </div>
                    `;
